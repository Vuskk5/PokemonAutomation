...
def test_api_berries_get(pokeapi):
    berry = pokeapi.berries.get('cheri')
    assert berry.id == 1


def test_api_berries_list(pokeapi):
    berries = pokeapi.berries.list(limit=30, offset=0).results
...
def test_api_berry_firmness_get(pokeapi):
    firmness = pokeapi.berry_firmnesses['very-soft']
    assert firmness.id == 1


def test_api_berry_firmness_list(pokeapi):
    resource_list = pokeapi.berry_firmnesses.list(limit=30, offset=0)
...
def test_api_berry_flavor_get(pokeapi):
    firmness = pokeapi.berry_flavors['1']
    assert firmness.id == 1


def test_api_berry_flavor_list(pokeapi):
    resource_list = pokeapi.berry_flavors.list(limit=30, offset=0)
    assert len(resource_list.results) == resource_list.count
